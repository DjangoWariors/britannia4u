...
def upload_csv_usere(request):
    ...
    with open(absolute_file_path, 'r') as file:
        ...
        for row in reader:
            ...
            status = True if row[27].strip() == 'Active' else False

            inserted_date_str = row[28].strip()

            if not inserted_date_str:
                continue
            ...
            user, created = User.objects.get_or_create(username=username, defaults={
                ...
                'udf1': udf1,
                'udf2': udf2,
                # 'password':make_password('crazibrain@b4u')
                'uuid': uuid.uuid4()
            })
...
def upload_csv(request):
    ...
    with open(absolute_file_path, 'r') as file:
        ...
        for row in reader:
            ...
            payout = RetailerPayout(user=user_obj, month=month, year=year, amount=amount, utr=utr, date=inserted_date, bank_name=bank_name, account_no=account_no, remark=remark, status=status)
            payout.save()
